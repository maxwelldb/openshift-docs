...
. For the remaining control plane machines, open the provider specs for editing. For example, to edit the second control plane machine, enter the following command:
+
[source,terminal]
----
...
. For the other control plane machines, edit the value of the `serverGroupName` property in their provider specs to match the first control plane machine in the set.
+
...
where:
+
`serverGroupName`:: Specifies the server group name. This value must be the same for all control plane machines in the set.
+
[NOTE]
====
