...
. For each control plane machine, edit the provider spec to match your environment. For example, to edit the first control plane machine, enter the following command. Replace the machine name in the command with a name from `oc get machines -n openshift-machine-api` if your cluster uses different host name patterns:
+
[source,terminal]
----
