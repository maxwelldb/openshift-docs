...
// TODO: Verify supportability of manifest modification.

[id="installation-osp-setting-worker-affinity_{context}"]
= Setting compute machine affinity

Optionally, you can set the affinity policy for compute machines during installation.

[TIP]
====
The installer does not select an affinity policy for compute machines by default. 
...
. From the newly generated manifest files, open the `MachineSet` definition file, which is in the `manifests` directory and is named `99_openshift-cluster-api_worker-machineset-0.yaml`.
...
The installer uses the `MachineSet` definition that you modified to create compute machines within your {rh-openstack} server group. 