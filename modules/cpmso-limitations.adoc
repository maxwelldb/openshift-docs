...
* Clusters that do not have preexisting machines that represent the control plane nodes cannot use a control plane machine set.
You cannot enable the control plane machine set after installation on those clusters.
...
* The Operator requires the Machine API Operator to be operational and is therefore not supported on clusters with manually provisioned machines.
When you install a {product-title} cluster with manually provisioned machines, your platform might create an active generated `ControlPlaneMachineSet` custom resource (CR).
If it does, remove the Kubernetes manifest files that define the control plane machine set.
Follow the removal steps from the installation documentation for your platform.
